...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // 1. Check Developer Fallback first to completely avoid network requests/fetch issues
    if (email.toLowerCase().trim() === 'nithishwaran2703@gmail.com' && password === 'admin123') {
    ...
    try {
      ...
      try {
        ...
        console.warn('Supabase Auth fetch failed/unreachable:', authFetchError);
      }

      setError('Invalid login credentials. (If Supabase is offline, use the fallback password: admin123)');
    } catch (err: any) {
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-10 border border-gray-100">
        ...
        <div className="mt-8 pt-8 border-t border-gray-100 text-center space-y-2">
          ...
          <p className="text-xs text-gray-400">
            If Supabase Auth is unreachable, sign in with: <br />
            Email: <span className="font-semibold text-gray-500">nithishwaran2703@gmail.com</span> <br />
            Password: <span className="font-semibold text-gray-500">admin123</span>
          </p>
        </div>
      </div>
    </div>
